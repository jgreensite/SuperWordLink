...
//	private float m_maximumTilt = 45f;
        private readonly float m_tiltSpeed = 1.0f;
        ...
        public bool rotateFaceUp;
        public bool rotateFlattenText;
        ...
        public void makeInHand()
        {
            ...
            //This sets the Layer
            //TODO - Consider using something like the following to set recursively the layer and do this on the. Array.ForEach(gameObject.GetComponentsInChildren<Transform>(), (entry) => entry.gameObject.layer = LayerMask.NameToLayer("MyLayer"));
            gameObject.layer = LayerMask.NameToLayer(CS.OBJ_LOCATION_LAYER_PLAYERHAND);
        }

        // Call this to make a card in the gameboard
        public void makeInGameboard()
        {
            //TODO - Consider using something like the following to set recursively the layer and do this on the. Array.ForEach(gameObject.GetComponentsInChildren<Transform>(), (entry) => entry.gameObject.layer = LayerMask.NameToLayer("MyLayer"));
            gameObject.layer = LayerMask.NameToLayer(CS.OBJ_LOCATION_LAYER_GAMEBOARD);
        }
        ...
        private void Start()
        {
            ...
            m_currentTextXRot = m_initialTextXRot =
                transform.Find("PlayingCardWordFront").transform.rotation.eulerAngles.x;
            m_currentTextYRot = m_initialTextYRot =
                transform.Find("PlayingCardWordFront").transform.rotation.eulerAngles.y;
            m_currentTextZRot = m_initialTextZRot =
                transform.Find("PlayingCardWordFront").transform.rotation.eulerAngles.z;

            Debug.Log("Card Front Text X, Y and Z rotations are " + m_currentXRot + " " + m_currentYRot + " " +
                      m_currentZRot);
...
//		gameObject.transform.Find ("Card").GetComponent<Renderer>().materials = mats;
        ...
        private void Update()
        {
            ...
            if (m_currentXRot > (m_initialXRot + m_finalXValue) * 0.98) m_currentXRot = m_initialXRot + m_finalXValue;

            //make sure that if Y is close the target angle of rotation it doesn't over or under rotate
            if (m_currentYRot > (m_initialYRot + m_finalYValue) * 0.98) m_currentYRot = m_initialYRot + m_finalYValue;

            //make sure that if Z is close the target angle of rotation it doesn't over or under rotate
            if (m_currentZRot > (m_initialZRot + m_finalZValue) * 0.98) m_currentZRot = m_initialZRot + m_finalZValue;
